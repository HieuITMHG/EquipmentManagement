rom models.database import get_connection
class ManagerService:
    ...
    @staticmethod
    def get_all_penalty_ticket():
        """Lấy tất cả các phiếu xử phạt"""
        conn = get_connection()
        cursor = conn.cursor(dictionary=True)
        try:
            cursor.callproc('get_all_penalty_ticket')  # Giả sử đã tạo procedure này trong DB
            ...
            print(f"[get_all_penalty_ticket Error: {e}]")
            return []
        finally:
            cursor.close()
            conn.close()
    ...
    @staticmethod
    def get_penalty_ticket_by_id(ticket_id):
        """Lấy thông tin một phiếu xử phạt theo ID"""
        conn = get_connection()
        cursor = conn.cursor(dictionary=True)
        try:
            query = "SELECT * FROM penalty_ticket WHERE id = %s"
            cursor.execute(query, (ticket_id,))
    ...
    @staticmethod
    def get_all_liquidation_slip():
        """Lấy tất cả các phiếu thanh lý"""
        conn = get_connection()
        cursor = conn.cursor(dictionary=True)
        try:
            cursor.callproc('get_all_liquidation_slip')  # Giả sử đã tạo procedure này trong DB
            ...
            print(f"[get_all_liquidation_slip Error: {e}]")
            return []
        finally:
            cursor.close()
            conn.close()
    ...
    @staticmethod
    def get_detail_liquidation_by_id(liquidation_id):
        """Lấy chi tiết một phiếu thanh lý theo ID"""
        conn = get_connection()
        cursor = conn.cursor(dictionary=True)
        try:
            query = "SELECT * FROM detail_liquidation_slip WHERE liquidation_slip_id = %s"
            cursor.execute(query, (liquidation_id,))
            return cursor.fetchall()  # Trả về list vì có thể nhiều thiết bị
        finally:
            cursor.close()
            conn.close()
